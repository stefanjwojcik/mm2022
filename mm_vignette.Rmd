...
2022-2023 season using the **scrape.game.results** function. In fact, you can
...
```{r, eval = FALSE}
games.2023 = scrape.game.results(2023, league = "women")
```
...
```{r, eval = FALSE}
pred.pop.2023 = scrape.population.distribution(2023, league = "women")
```

We've made life even easier for you by pre-scraping **games.women.2023**,
**pred.pop.women.2023**, **games.men.2023** and **pred.pop.men.2023**. These are
all available as datasets in the package. You're welcome.

```{r}
head(games.women.2023)
head(pred.pop.women.2023)
```
...
```{r}
set.seed(1)
prob.matrix = bradley.terry(games = games.women.2023)
...
```{r}
head(pred.538.women.2023)
```

### Simulating the tournament

We've already taken care of those pesky first four games for you and stored the
bracket in a vector called **bracket.women.2023**. This lists the tournament
teams in order of overall seed.

```{r}
head(bracket.women.2023)
```
...
set.seed(2023)
outcome = sim.bracket(
  bracket.empty = bracket.women.2023,
  prob.matrix = prob.matrix
)
draw.bracket(bracket.empty = bracket.women.2023, bracket.filled = outcome)
```

Congratulations are in order for Indiana, for winning the `set.seed(2023)`
NCAA women's basketball tournament!

### Finding a good bracket

That's all well and good for Indiana, but wouldn't you prefer to win
...
draw.bracket(bracket.empty = bracket.women.2023, bracket.filled = my.bracket)
```
...
There you have it. You can expect this bracket to win 7.1% of the Groundhog
